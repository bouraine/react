...
import { listMonthNames, Month } from "./typings";
...
export const getYearMonths = (year: number): Month[] =>
  range(0, 12).reduce((acc, curr, i) => {
    ...
    return [
      ...acc,
      { name: listMonthNames[day.getMonth()], firstDate: day.getDay(), nbDays, id: i, year }
    ];
  }, [] as Month[]);

export const isWeekend = (firstDay: number, i: number) =>
  (i + firstDay + 1) % 7 === 0 || (i + firstDay + 1) % 7 === 1;
